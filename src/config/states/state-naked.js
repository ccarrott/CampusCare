...
export async function loadNakedState() {
  ...
  await query("INSERT INTO Nurse (StaffNumber, FirstName, LastName, PhoneNumber, Password, Email, ClinicID, Bio, YearsExperience) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ['NUR001', 'Sarah', 'Jenkins', '0821234567', nursePw, 'sarah.jenkins@mandela.ac.za', 'CLN001', 'Experienced campus nurse specialising in student wellness and primary care.', 8]);
  await query("INSERT INTO Nurse (StaffNumber, FirstName, LastName, PhoneNumber, Password, Email, ClinicID, Bio, YearsExperience) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ['NUR002', 'David', 'Khumalo', '0839876543', nursePw, 'david.khumalo@mandela.ac.za', 'CLN001', 'Focused on mental health support and holistic student care.', 5]);
  await query("INSERT INTO Nurse (StaffNumber, FirstName, LastName, PhoneNumber, Password, Email, ClinicID, Bio, YearsExperience) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ['NUR003', 'Thandiwe', 'Nkosi', '0841112233', nursePw, 'thandiwe.nkosi@mandela.ac.za', 'CLN002', 'Sports medicine background. Great with physical assessments.', 12]);

  console.log('[State: Naked] Complete. 1 admin + 3 nurses seeded. System ready for fresh use.');
  return { success: true, message: 'Naked state loaded. 1 admin, 3 nurses, all tables clear.' };
}
